...
[![Version](https://img.shields.io/badge/version-2.2.74-green.svg)](CHANGELOG.md)
...
> 🆕 **What's New in v2.2.74**: Repository maintenance v2.2.74. Verified all ecosystem cross-references and bilingual consistency. Refreshed last-updated date.
...
- [中文说明](#中文说明)
  ...
  - [💡 11 大核心能力（8 + 3 CEO 视角）](#-11-大核心能力8--3-ceo-视角)
  - [🔧 实用示例](#-实用示例)
  - [📁 项目结构](#-项目结构)
...
### ⚠️ 常见数据叙事陷阱 (Common Pitfalls)

| 陷阱 | 表现 | 应对 |
|------|------|------|
| 跳过了上下文 | 直接选图表，没想清楚受众和目的 | 永远从 `build_context()` 开始 |
| 展示探索性数据 | 把分析过程当成最终汇报 | 只展示珍珠，不展示牡蛎 |
| 颜色滥用 | 每种数据系列用不同颜色 | 灰色基底 + 单色强调，一图不超过 2-3 色 |
| 描述性标题 | "2024 销售趋势" | 行动标题："Q4 销售额下降 15%，需采取行动" |
| 非零基线柱状图 | Y 轴从 50 开始，扭曲数据比例 | 柱状图必须从零基线开始 |
| 独立图例 | 图例和图表分离，受众来回对照 | 直接在数据旁标注 |

> 💡 **提示**: SWD 黄金法则——每张图只讲一个故事。如果你的图表需要超过 5 秒才能理解，它太复杂了。
...
本技能是 **AliDujie UX 研究技能生态系统** 的数据叙事层，与其他技能无缝协作：
...
👉 **探索完整生态系统**: [UDM](https://github.com/AliDujie/universal-design-methods) · [JTBD](https://github.com/AliDujie/jtbd-knowledge-skill) · [Persona](https://github.com/AliDujie/web-persona-skill) · [QuantUX](https://github.com/AliDujie/Quantitative-UX-Research) · [VPD](https://github.com/AliDujie/value-proposition-design)
...
> 💡 **Quick Tip**: `Persona → JTBD → UDM → QuantUX → VPD → SWD → STM` — a complete research pipeline from user definition to data-driven presentation and strategic analysis. Start with any skill, then chain to the next.
...
*Last Updated: 2026-05-15 | AliDujie Skill Ecosystem | v2.2.74*