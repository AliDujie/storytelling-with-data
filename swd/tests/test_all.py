...
def test_build_decision_comparison():
    ...
    assert "决策选项对比" in result_default, "应包含标题"
    assert "方案 A" in result_default, "应包含方案A"
    assert "方案 B" in result_default, "应包含方案B"
    assert "方案 C" in result_default, "应包含方案C"
    assert "投入成本" in result_default, "应包含投入成本维度"
    assert "预期回报" in result_default, "应包含预期回报维度"
    assert "ROI" in result_default, "应包含ROI维度"
    assert "风险等级" in result_default, "应包含风险等级维度"
    assert "推荐方案" in result_default, "应包含推荐方案"
    assert "关键假设" in result_default, "应包含关键假设"
    assert "决策建议" in result_default, "应包含决策建议"
    
    # 测试自定义选项
    custom_options = [
        {"name": "方案 X", "投入": "500 万", "预期回报": "1200 万/年", "ROI": "240%", "风险": "低", "时间": "3 个月", "可逆性": "可逆", "推荐度": "⭐⭐⭐⭐⭐"},
        {"name": "方案 Y", "投入": "800 万", "预期回报": "1500 万/年", "ROI": "187%", "风险": "中", "时间": "6 个月", "可逆性": "可逆", "推荐度": "⭐⭐⭐⭐"},
    ]
    result_custom = skill.build_decision_comparison(custom_options)
    assert "方案 X" in result_custom, "应包含自定义方案X"
    assert "方案 Y" in result_custom, "应包含自定义方案Y"
    assert "500 万" in result_custom, "应包含自定义投入"
    assert "240%" in result_custom, "应包含自定义ROI"
    
    print("✅ test_build_decision_comparison passed")


# ────────────────────────────────────────────
# 测试 10：CEO 视角 - 执行风险可视化
# ────────────────────────────────────────────
def test_visualize_execution_risks():
    skill = SWDSkill("风险评估")
    result = skill.visualize_execution_risks()
    
    assert isinstance(result, str), "visualize_execution_risks 应返回字符串"
    assert "执行风险可视化" in result, "应包含标题"
    assert "风险矩阵" in result, "应包含风险矩阵"
    assert "影响程度" in result, "应包含影响程度维度"
    assert "发生概率" in result, "应包含发生概率维度"
    assert "方案 A" in result, "应包含方案A"
    assert "方案 B" in result, "应包含方案B"
    assert "方案 C" in result, "应包含方案C"
    assert "风险对比" in result, "应包含风险对比表"
    assert "风险缓解措施" in result, "应包含风险缓解措施"
    assert "缓解措施" in result, "应包含具体缓解措施"
    assert "负责人" in result, "应包含负责人"
    assert "检查点" in result, "应包含检查点"
...
def test_generate_decision_framework():
    skill = SWDSkill("决策框架")
    result = skill.generate_decision_framework()
    
    assert isinstance(result, str), "generate_decision_framework 应返回字符串"
    assert "数据叙事决策框架" in result, "应包含标题"
    assert "决策质量检查清单" in result, "应包含决策质量检查清单"
    assert "数据质量" in result, "应包含数据质量维度"
    assert "逻辑严密" in result, "应包含逻辑严密维度"
    assert "风险披露" in result, "应包含风险披露维度"
    assert "可执行性" in result, "应包含可执行性维度"
    assert "决策质量评分" in result, "应包含决策质量评分"
    assert "决策后追踪" in result, "应包含决策后追踪"
    assert "复盘时间" in result, "应包含复盘时间"
    assert "决策流程图" in result, "应包含决策流程图"
    
    print("✅ test_generate_decision_framework passed")


# ────────────────────────────────────────────
# 测试 12：CEO 视角 - 完整故事生成（含 CEO 分析）
# ────────────────────────────────────────────
def test_build_story_with_ceo_analysis():
    skill = SWDSkill("业务增长数据分析")
    
    # 测试不包含 CEO 分析
    result_no_ceo = skill.build_story(
        protagonist="产品团队",
        imbalance="用户增长放缓",
        include_ceo_analysis=False,
    )
    assert isinstance(result_no_ceo, str), "build_story 应返回字符串"
    assert "决策选项对比" not in result_no_ceo, "不包含CEO分析时不应有决策对比"
    assert "执行风险可视化" not in result_no_ceo, "不包含CEO分析时不应有风险可视化"
    assert "数据叙事决策框架" not in result_no_ceo, "不包含CEO分析时不应有决策框架"
    
    # 测试包含 CEO 分析（默认选项）
    result_with_ceo = skill.build_story(
        protagonist="产品团队",
        imbalance="用户增长放缓",
        include_ceo_analysis=True,
    )
    assert isinstance(result_with_ceo, str), "build_story 应返回字符串"
    assert "决策选项对比" in result_with_ceo, "包含CEO分析时应有决策对比"
    assert "执行风险可视化" in result_with_ceo, "包含CEO分析时应有风险可视化"
    assert "数据叙事决策框架" in result_with_ceo, "包含CEO分析时应有决策框架"
    assert "CEO 视角扩展分析" in result_with_ceo, "应包含CEO视角标题"
    
    # 测试包含 CEO 分析（自定义选项）
    custom_options = [
        {"name": "方案 A", "投入": "300 万", "预期回报": "600 万/年", "ROI": "200%", "风险": "低", "时间": "2 个月", "可逆性": "可逆", "推荐度": "⭐⭐⭐⭐⭐"},
        {"name": "方案 B", "投入": "500 万", "预期回报": "900 万/年", "ROI": "180%", "风险": "中", "时间": "4 个月", "可逆性": "可逆", "推荐度": "⭐⭐⭐⭐"},
    ]
    result_custom_ceo = skill.build_story(
        protagonist="产品团队",
        imbalance="用户增长放缓",
        include_ceo_analysis=True,
        decision_options=custom_options,
    )
    assert "方案 A" in result_custom_ceo, "应包含自定义方案A"
    assert "方案 B" in result_custom_ceo, "应包含自定义方案B"
    assert "300 万" in result_custom_ceo, "应包含自定义投入"
    assert "200%" in result_custom_ceo, "应包含自定义ROI"
    
    print("✅ test_build_story_with_ceo_analysis passed")
...
if __name__ == "__main__":
    tests = [
        ...
        test_build_story_with_ceo_analysis,
    ]
